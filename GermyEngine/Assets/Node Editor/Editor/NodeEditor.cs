...
public class NodeEditor : EditorWindow {
	...
	private static void Init() {
		EditorWindow w = EditorWindow.GetWindow(typeof(NodeEditor), false,"Node Editor");
		w.Show();
	}
	#endregion
	...
	private void DragNode(int i) {
		...
		if (isDraggingNode) {
			...
			currentlyDragging.pos.position = Vector2.Lerp(currentlyDragging.pos.position, new Vector2(e.mousePosition.x - currentlyDragging.pos.size.x / 2, e.mousePosition.y - currentlyDragging.pos.size.y / 2),.1f * Time.deltaTime); 
		}
